...
public class Board {
    ...
    public boolean wayIsClear(int row, int col, int row1,int col1)  // 1 0 3 0
    {
        int i = row;
        int j = col;


        if(i-row1==0) // ход горизонтально(ладья, ферзь)
        {
            if(j-col1==0)
            {
                return false;
            }
            if(Math.abs(j-col1) == 1){return true;}
            if(j-col1 < 0) // горизонтально вправо
            {
                for(int k = j+1; k < col1;k++)
                {
                    if(!(fields[i][k] == null))
                    {
                        return false;
            ...
            if(j-col1 > 0) // горизонтально влево
            {
                for(int k = j-1; k > col1;k--)
                {
                    if(!(fields[i][k] == null))
        ...
        if(j-col1==0) // ход по вертикали(ладья, ферзь)
        {
            if(Math.abs(i-row1) == 1){return true;}
            if(i- row1< 0) // вертикально вверх
            {
                for(int l = i+1; l < row1;l++)
                {
                    if(!(fields[l][j] == null))
                    {
                        return false;
        ...
        if(i-row1<0) // ход по диагонали (ферзь, слон) вверх
        {
            if( Math.abs(j-col1) == 1 && Math.abs(i-row1)==1 ){return true;}
            if(j-col1 < 0) // вправо
            {
                for(int l = i+1, k = j+1; k < col1;k++,l++)
                {
                    if(!(fields[l][k] == null))
                    {
                        return false;
